...
def test_tree(tree):
    with open('./data/test.pkl', 'rb') as fp:
        ...
        F_test = data_test['F']
        y_test = data_test['y']
        test_data = [X_test.T[i].tolist() for i in range(y_test.T.shape[0]) if y_test.T[i] == 1]
        test_label = [y_test.T[i] for i in range(y_test.T.shape[0]) if y_test.T[i] == 1]
...
class GRUTree(object):
    ...
    def train(self, X_train, F_train, y_train, iters_retrain=25, num_iters=1000,
        ...
        npr.seed(42)
        num_retrains = 2# num_iters // iters_retrain
        for i in range(num_retrains):
            ...
            self.mlp.train(W_train[:self.gru.num_weights, :], APL_train, num_iters=750,
...
class GRU(object):
    ...
    def fit_tree(self, weights, X_train, F_train, y_train):
        ...
        # print(y_train_hat_int)
        tree.fit(X_train.T, y_train_hat_int.T)
        # print(tree.score(X_train.T,y_train.T))
        #test the current decisionTree
        score = test_tree(tree)
        if score > 0.95:
            visualize(tree, './trained_models/tree_best.pdf')
            with open('dt_best.pkt', 'wb') as file:
...
class MLP(object):
    ...
    def train(self, X_train, y_train, batch_size=32, num_iters=750, 
              lr=1e-3, param_scale=0.01, log_every=100, init_weights=None):
...
def visualize(tree, save_path):
    ...
    """
    dot_data = export_graphviz(tree, out_file=None, impurity = False,
                               filled=True, rounded=True, proportion = True, class_names = ['Benign', 'Malicious'])
    graph = pydotplus.graph_from_dot_data(dot_data)
    graph = make_graph_minimal(graph)  # remove extra text
    with open('dt.pkt', 'wb') as file:
        pickle.dump(tree, file)
...
if __name__ == "__main__":
    ...
    # print(F_train)
    gru = GRUTree(100, 128, [64], 1, strength=args.strength)
    ...
    tree_dr = DecisionTreeClassifier(min_samples_leaf=25)
    test_tree(tree_dr)
    ...
    #     tree_dr.fit(X_train.T, y_train.T)
    #     print(tree_dr.score(X_test.T,y_test.T))
